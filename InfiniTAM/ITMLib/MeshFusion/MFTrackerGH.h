...
_CPU_AND_GPU_CODE_ inline bool computePerPointGH_Depth_Ab(THREADPTR(float) *A, THREADPTR(float) &b,
	int idx, std::vector<cv::Point3f>& diff, std::vector<cv::Point3f>& curr, std::vector<cv::Point3f> &currNormal,float distThresh)
	...
	//const CONSTPTR(Vector4f) *normalsMap, float distThresh)
{
	...
	cv::Point3f ptDiff=diff[idx];
	cv::Point3f tmp3Dpoint = curr[idx];
	cv::Point3f corr3Dnormal=currNormal[idx];

	if ((corr3Dnormal.x == 0) && (corr3Dnormal.y == 0) && (corr3Dnormal.z == 0))
		return false;
	...
	const CONSTPTR(Vector4f) *normalsMap, float distThresh)*/
{
	...
	bool ret = computePerPointGH_Depth_Ab<shortIteration, rotationOnly>(A, b, idx, diff, curr, currNormal, distThresh);// x, y, depth, viewImageSize, viewIntrinsics, sceneImageSize, sceneIntrinsics, approxInvPose, scenePose, pointsMap, normalsMap, distThresh);
